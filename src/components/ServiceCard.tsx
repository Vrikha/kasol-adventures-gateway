...
const ServiceCard = ({ service, onWhatsApp, onEmail }: ServiceCardProps) => {
  return (
    <Card className="group bg-white/95 backdrop-blur-md border border-yellow-200/50 hover:bg-white hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden shadow-xl">
      <div className="relative h-80 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex items-center space-x-2">
          <Badge className="bg-yellow-600 hover:bg-yellow-700 backdrop-blur-md border border-yellow-500/50 text-white font-semibold">
            {service.category}
          </Badge>
        ...
        <div className="grid grid-cols-2 gap-3">
          {service.features.map((feature, idx) => (
            <div key={idx} className="flex items-center space-x-2 text-sm text-gray-700">
              <div className="w-2 h-2 bg-gradient-to-r from-yellow-500 to-amber-600 rounded-full flex-shrink-0" />
              <span>{feature}</span>
            </div>
          ))}
        </div>
